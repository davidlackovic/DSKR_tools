...
class Combine3D():
    ...
    def update_solver(self):
        ...
        # L je definiran znotraj self.combine()
        self.eig_vec = self.L @ eig_vec_q

    def find_shared_nodes(self, tol=1e-6):
        vsa_vozlisca = []
        for obj_idx, obj in enumerate(self.objects):
            for node_idx, coord in enumerate(obj.nodes):
                vsa_vozlisca.append((obj_idx, node_idx, coord))
        ...
        for i in range(len(vsa_vozlisca)):
            if used[i]: continue
            current_group = [vsa_vozlisca[i][:2]]
            used[i] = True
            for j in range(i + 1, len(vsa_vozlisca)):
                if not used[j]:
                    dist = np.linalg.norm(vsa_vozlisca[i][2] - vsa_vozlisca[j][2])
                    if dist < tol:
                        current_group.append(vsa_vozlisca[j][:2])
                        used[j] = True
            if len(current_group) > 1:
                node_groups.append({'global_id': global_id_counter, 'nodes': current_group})
                global_id_counter += 1
        return node_groups
    ...
    def combine(self):
        ...
        for group in self.skupna_vozlisca:
            nodes = group['nodes']
            m_obj_idx, m_node_idx = nodes[0]
            
            # 3D: Frame ima 6 PS, Truss ima 3 PS
            m_dpa = 6 if self.objects[m_obj_idx].type == 'frame' else 3
            m_base = offsets[m_obj_idx] + m_node_idx * m_dpa
            
            for s_obj_idx, s_node_idx in nodes[1:]:
                s_dpa = 6 if self.objects[s_obj_idx].type == 'frame' else 3
                s_base = offsets[s_obj_idx] + s_node_idx * s_dpa
                
                # Translacije (u, v, w) - vedno povezane
                for d in range(3):
                ...
                # Rotacije (rx, ry, rz) - samo če sta oba frame in je rigid
                if self.connection_type == "rigid" and m_dpa == 6 and s_dpa == 6:
                    for d in range(3, 6):
                        row = np.zeros(total_dofs)
                        row[m_base + d] = 1
                        row[s_base + d] = -1
    ...
    def animate_mode_shapes(self, scale=1.0):
        ...
        for obj in self.objects:
            all_nodes.append(obj.nodes) # obj.nodes morajo biti (N, 3)
            all_elements.extend(obj.elements + node_offset)
            node_offset += len(obj.nodes)

        pts_orig = np.vstack(all_nodes)
        lines = [[e[0], e[1]] for e in all_elements]
        ...
        vis = o3d.visualization.VisualizerWithKeyCallback()
        vis.create_window(window_name="Combine3D - Modal Analysis", width=1200, height=800)
        vis.add_geometry(line_set_orig); vis.add_geometry(line_set_deformed); vis.add_geometry(points_cloud)
        
        opt = vis.get_render_option()
        opt.background_color = np.array([1, 1, 1])
        opt.line_width = 10.0; opt.point_size = 12.0
        ...
        def get_displacement(m_idx):
            ...
            for i, obj in enumerate(self.objects):
                start, end = self.offsets[i], self.offsets[i+1]
                dpa = 6 if obj.type == 'frame' else 3
                obj_mode = mode[start:end]
                # Vzamemo prve tri PS (u, v, w) za vsako vozlišče
                disp_list.append(np.column_stack((obj_mode[0::dpa], obj_mode[1::dpa], obj_mode[2::dpa])))
        ...
        vis.register_key_callback(ord(' '), lambda v: exec("state['animate'] = not state['animate']"))
        vis.register_key_callback(262, next_mode); vis.register_key_callback(263, prev_mode)
